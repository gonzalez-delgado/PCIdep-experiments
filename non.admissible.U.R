...
# This code reproduces the numerical analysis of Section 5.4.1 and Figures 5, 13.
...
deltaseq <- c(4,6,8,10) # Separation between clusters
...
for(delta in deltaseq){
  ...
  for(j in 4:6){
    ...
    if(j == 4){global_null_D4[[delta]] <- sim}
    if(j == 5){global_null_D5[[delta]] <- sim}
    if(j == 6){global_null_D6[[delta]] <- sim}
...
data_D4 <- as.data.frame(do.call('rbind', global_null_D4)); colnames(data_D4) <- c('pv_av','effect_av','pv_cen','effect_cen','pv_sin','effect_sin','pv_com','effect_com','pv_km','effect_km','delta')
data_D5 <- as.data.frame(do.call('rbind', global_null_D5)); colnames(data_D5) <- c('pv_av','effect_av','pv_cen','effect_cen','pv_sin','effect_sin','pv_com','effect_com','pv_km','effect_km','delta')
data_D6 <- as.data.frame(do.call('rbind', global_null_D6)); colnames(data_D6) <- c('pv_av','effect_av','pv_cen','effect_cen','pv_sin','effect_sin','pv_com','effect_com','pv_km','effect_km','delta')

title_D4 <-  'U = Toeplitz'
title_D5 <-  expression(paste('U = AR(3) with ',beta[1],beta[2],"<",0))
title_D6 <-  "U = Banded"
sublist <- list()
